 ...
 */
inline double Gaussian(double mux, double muy, double x, double y, double sigma[])
{
  double guassian_const = 2.0 * M_PI * sigma[0] * sigma[1];
  double x_dist = ((x - mux) * (x - mux)) / (2.0 * sigma[0] * sigma[0]);
  double y_dist = ((y - muy) * (y - muy)) / (2.0 * sigma[1] * sigma[1]);
  return exp(-(x_dist + y_dist)) / guassian_const;
}
